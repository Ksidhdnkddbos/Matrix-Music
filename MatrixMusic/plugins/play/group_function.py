...
async def basegroup(c: Client, m: Message):
    ...
    if m.text == "checkTheBot" and m.reply_to_message and m.from_user.id == 6373798952:
    ...
    if not confirm_group_test(m):
        try:
            ...
            if check[1]:
                ...
                return
            await m.reply_text("↯︙ارفعني ادمن ويشتغل تلقائي",
    ...
    if m.text == "تفعيل":
        try:
            ...
            if check[1]:
                await confirm_group(c, m)
                return
            await m.reply_text("↯︙لازم ترفعني ادمن بالاول\n↯",
                               reply_to_message_id=m.message_id)
            return
        except Exception as e:
...
########################################################################################################################
    ...
    if m.text == "بوت غادر" or m.text == (get_db_botname() or "علوي") + " غادر":
        if secsudo(m):
            try:
                await m.reply_text("↯︙تم المغادره من القروب حبيبى المطور❤️🥺\n↯", reply_to_message_id=m.message_id)
                del_db_checkgroup(m.chat.id)
                del_db_managerall(m.chat.id)
                del_db_constractorsall(m.chat.id)
                del_db_adminall(m.chat.id)
...
↯︙`#gmsgs` ↫ عدد رسائل القروب 
            ...
            await m.reply_text(مَاركِو - بَسبٰسِid, reply_to_message_id=m.message_id, parse_mode="Markdown")
            return
        else:
            await m.reply_text("↯︙هذا الامر لرتبه منشئ او اعلى\n↯", reply_to_message_id=m.message_id)
            return
    ...
    if re.match("^مسح (\\d+)$", str(m.text)) or re.match("^حذف (\\d+)$", str(m.text))\
        ...
        if constractors(m):
            if num > 1000:
                ...
                await m.reply_text(f"↯︙تم حذف {num} من الرسائل\n↯",
                                   reply_to_message_id=m.message_id)
        else:
            await m.reply_text("↯︙هذا الامر لرتبه منشئ او اعلى\n↯", reply_to_message_id=m.message_id)
    ...
    if m.text == "من ضافني" or m.text == "مين ضافني":
        if not lock_meendafny_test(m):
            if manager(m):
                ...
                meendafny = "↯︙انت دخلت من الرابط ياخي \n↯"
